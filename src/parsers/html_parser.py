...
class HtmlParser(BaseParser):
    async def parse(self, path: Path, options: dict | None = None) -> ParseResult:
        ...
        for elem in body.find_all(
            ["h1", "h2", "h3", "h4", "h5", "h6", "p", "ul", "ol", "pre", "code", "table", "img"], recursive=True
        ):
            if elem.name in ("h1", "h2", "h3", "h4", "h5", "h6"):
                ...
                if text_val:
                    sections.append(
                        Section(
                            index=section_idx, type=SectionType.PARAGRAPH, content=text_val, page=1, confidence=0.85
                        )
                    )
                    section_idx += 1
                ...
                if items:
                    sections.append(
                        Section(
                            index=section_idx, type=SectionType.LIST, content="\n".join(items), page=1, confidence=0.85
                        )
                    )
                    section_idx += 1
                ...
                if text_val:
                    sections.append(
                        Section(index=section_idx, type=SectionType.CODE, content=text_val, page=1, confidence=0.8)
                    )
                    section_idx += 1
                ...
                sections.append(
                    Section(
                        index=section_idx, type=SectionType.IMAGE, content="", images=[img_ref], page=1, confidence=1.0
                    )
                )
                image_idx += 1
                section_idx += 1
        ...
        if title and not any(s.type == SectionType.HEADING for s in sections):
            sections.insert(
                0, Section(index=0, type=SectionType.HEADING, content=title, page=1, level=1, confidence=0.9)
            )
        ...
        if not any(
            s.type in (SectionType.HEADING, SectionType.PARAGRAPH, SectionType.LIST, SectionType.CODE) for s in sections
        ):
            sections.append(
                Section(
                    index=section_idx, type=SectionType.PARAGRAPH, content=markdown_text.strip(), page=1, confidence=0.8
                )
            )
        ...
        metadata = DocumentMetadata(
            ...
            has_toc=bool(title) or any(s.type == SectionType.HEADING for s in sections),
            toc=[],
